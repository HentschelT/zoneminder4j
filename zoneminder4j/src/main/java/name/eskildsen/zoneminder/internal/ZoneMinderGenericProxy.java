...
public class ZoneMinderGenericProxy implements IZoneMinderResponse {
    ...
    protected ZoneMinderSession aquireSession()
            throws FailedLoginException, IOException, ZoneMinderUrlNotFoundException {
        if (_session != null) {
            return _session;
        }
        return null;
    ...
    private URI BuildURI(IZoneMinderConnectionInfo connectionInfo, String methodPath, String queryString)
            throws MalformedURLException {
        String s = "";

        // Build Path to the required method in the API
        UriBuilder uriBuilder = null;
        uriBuilder = UriBuilder.fromUri(connectionInfo.getZoneMinderRootUri())
                .path(ZoneMinderServerConstants.SUBPATH_API).path(methodPath);

        if ((queryString != null) && (queryString != "")) {
            uriBuilder = uriBuilder.replaceQuery(queryString);
        }
        URI result = uriBuilder.build();
        this.debugURI = result.toASCIIString();
        return result;
    }
    ...
    protected String sendPut(String methodPath, String postParams) throws Exception {
        ...
        String result = session._sendRequest(BuildURI(session.getConnectionInfo(), methodPath), HttpRequest.PUT,
                postParams);
        setHttpResponse(session);
    ...
    protected String sendPost(String methodPath, String postParams) throws Exception {
        ZoneMinderSession session = aquireSession();
        String result = session._sendRequest(BuildURI(session.getConnectionInfo(), methodPath), HttpRequest.POST,
                postParams);
        setHttpResponse(session);
        releaseSession(session);
    ...
    protected JsonObject getAsJson(String methodPath)
            throws FailedLoginException, ZoneMinderUrlNotFoundException, IOException {
        return getAsJson(methodPath, null);
    }

    protected JsonObject getAsJson(String methodPath, String queryString)
            throws FailedLoginException, ZoneMinderUrlNotFoundException, IOException {
