...
public class ZoneMinderMonitorProxy extends ZoneMinderGenericProxy implements IZoneMinderMonitor {
    ...
    public IZoneMinderDaemonStatus getCaptureDaemonStatus() {
        return (IZoneMinderDaemonStatus) getMonitorStatus(DAEMON_NAME_CAPTURE,
                ZoneMinderMonitorCaptureDaemonStatus.class);
    }

    @Override
    public IZoneMinderDaemonStatus getAnalysisDaemonStatus() {
        return (IZoneMinderDaemonStatus) getMonitorStatus(DAEMON_NAME_ANALYSIS,
    ...
    private ZoneMinderDaemonStatus getMonitorStatus(String daemonName, Type classType) {
        ...
        try {
            String strCommand = resolveCommands(ZoneMinderServerConstants.SUBPATH_API_MONITOR_DAEMONSTATUS_JSON,
                    "MonitorId", getId());
        ...
        try {
            ...
            for (Integer curPageIdx = 1; curPageIdx <= maxPages; curPageIdx++) {
                ...
                jsonObject = getAsJson(
                        resolveCommands(ZoneMinderServerConstants.SUBPATH_API_EVENTS_SPECIFIC_MONITOR_JSON, "MonitorId",
                                getId()),
                ...
                // ZoneMinderPagination pagination = gson.fromJson(jsonObject.getAsJsonObject("pagination"),
                // ZoneMinderPagination.class);
                ...
                for (JsonElement curJsonEvent : jsonEvents) {
                    ...
                    // list.add(gson.fromJson(((JsonObject)curJsonEvent).getAsJsonObject("Event"),
                    // ZoneMinderEvent.class));

                    list.add((ZoneMinderEvent) convertToClass(((JsonObject) curJsonEvent).getAsJsonObject("Event"),
                            ZoneMinderEvent.class));
    ...
    public IZoneMinderEventData getEventById(String eventId) {
        ...
        try {
            ...
            for (Integer curPageIdx = 1; curPageIdx <= maxPages; curPageIdx++) {
                ...
                for (JsonElement curJsonEvent : jsonEvents) {
                    // ZoneMinderEvent curEvent = gson.fromJson(((JsonObject)curJsonEvent).getAsJsonObject("Event"),
                    // ZoneMinderEvent.class);
                    ZoneMinderEvent curEvent = (ZoneMinderEvent) convertToClass(
                            ((JsonObject) curJsonEvent).getAsJsonObject("Event"), ZoneMinderEvent.class);
     ...
     */
    @Override
    public void activateForceAlarm(Integer priority, String reason, String note, String showText,
            Integer timeoutSeconds) throws IOException, FailedLoginException, ZoneMinderUrlNotFoundException {
        ZoneMinderSession session = aquireSession();

        ZoneMinderTelnetRequest request = new ZoneMinderTelnetRequest(TelnetAction.ON, getId(), priority, reason, note,
                showText, timeoutSeconds);
    ...
    public void deactivateForceAlarm() throws Exception {
        ...
        ZoneMinderTelnetRequest request = new ZoneMinderTelnetRequest(TelnetAction.OFF, getId(), 255, "", "", "", 0);
        session.writeTelnet(request.toCommandString());
        releaseSession(session);
    }
    ...
    public void SetEnabled(boolean enabled) {
        ...
        try {
            methodPath = resolveCommands(ZoneMinderServerConstants.SUBPATH_API_MONITOR_SPECIFIC_JSON, "MonitorId",
                    getId());
            if (enabled) {
    ...
    public IZoneMinderMonitorData getMonitorData() {
        ...
        JsonObject jsonObject = null;
        try {
            String command = resolveCommands(ZoneMinderServerConstants.SUBPATH_API_MONITOR_SPECIFIC_JSON, "MonitorId",
                    getId());
            JsonObject obj = getAsJson(command);
            jsonObject = getAsJson(
                    resolveCommands(ZoneMinderServerConstants.SUBPATH_API_MONITOR_SPECIFIC_JSON, "MonitorId", getId()))
    ...
    public ZoneMinderMonitorStatusEnum getMonitorDetailedStatus() {
        ...
        try {
            String strCommand = resolveCommands(ZoneMinderServerConstants.SUBPATH_API_MONITOR_ALARM_JSON, "MonitorId",
                    getId());
